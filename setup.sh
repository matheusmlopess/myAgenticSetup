...
CONFIG_DIR="${WSL_SETUP_CONFIG_DIR:-$HOME/.config/wsl-setup}"
...
write_runtime_config() {
  if $DRY_RUN; then
    echo "  [dry-run] would write runtime config to $CONFIG_FILE"
    return
  fi
  ...
  mkdir -p "$CONFIG_DIR"
  {
    ...
    printf 'export WSL_SETUP_DIR=%q\n' "$SCRIPT_DIR"
    printf 'export WSL_SETUP_SYNC_SCRIPT=%q\n' "$SCRIPT_DIR/sync.sh"
    printf 'export WSL_SETUP_CHECK_SCRIPT=%q\n' "$SCRIPT_DIR/check.sh"
    printf 'export WSL_SETUP_VERIFY_SCRIPT=%q\n' "$SCRIPT_DIR/verify.sh"
    printf 'export WSL_SETUP_LAST_SYNC_FILE=%q\n' "$SCRIPT_DIR/.last_sync.local"
    printf 'export WSL_SETUP_TRACKED_LAST_SYNC_FILE=%q\n' "$SCRIPT_DIR/.last_sync"
  } > "$CONFIG_FILE"
}
...
write_wrapper_script() {
  ...
  local wrapper_path="$LOCAL_BIN_DIR/$wrapper_name"

  if $DRY_RUN; then
    echo "  [dry-run] would write helper command $wrapper_path"
    return
  fi
...
CONFIG_FILE="\${WSL_SETUP_CONFIG_FILE:-\$HOME/.config/wsl-setup/config.sh}"
if [ -r "\$CONFIG_FILE" ]; then
  # shellcheck disable=SC1090
  . "\$CONFIG_FILE"
fi

TARGET_ROOT="\${WSL_SETUP_DIR:-}"
TARGET_SCRIPT="\$TARGET_ROOT/$target_script"

if [ -z "\$TARGET_ROOT" ] || [ ! -f "\$TARGET_SCRIPT" ]; then
  echo "$wrapper_name: unable to locate the repo. Re-run setup.sh from the repo root." >&2
  exit 1
fi
...
write_runtime_config
write_wrapper_script "wsl-sync" "sync.sh"
write_wrapper_script "wsl-check" "check.sh"
write_wrapper_script "wsl-verify" "verify.sh"
info "Runtime config refreshed at ~/.config/wsl-setup/config.sh"
info "Helper commands installed: wsl-sync, wsl-check, wsl-verify"
